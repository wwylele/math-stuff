...
\[
\upsilon_{s,t}(x) =^{\star}  \sum_r \frac{ \rho_{s,t}(s e^{t\rho_{s,t}} + t e^{s\rho_{s,t}}) }{r(s e^{tr} + t e^{sr})} e^{(r - \rho_{s,t})x}
\]
Remember $r= \rho_{s,t}$ is one of the root, whose corresponding term will be 1, so equivalently we want to prove the limit 
\[
...
We can also try extrapolate from the rational case. By convention we often say $\gcd(s, t) = 0$ for $s/t\notin\mathbb{Q}$. If we consider $\kappa\to0$, then
\[
\limsup_{x\to\infty}  \upsilon_{s,t}(x) = \liminf_{x\to\infty}  \upsilon_{s,t}(x) = 1
\]
which suggests the unified limit for irrational case.
...
\end{lemma}
\begin{proof}
	(Only for $s/t\in\mathbb{Q}$ for now. TODO for irrational)
	...
	For $s/t\notin\mathbb{Q}$, assuming the $\eta$ asymptote for irrational holds, then
	\[
	\lim_{i\to\infty} \frac{\eta(x_i - t)}{\eta_i} = \lim{i\to\infty} \frac{e^{\rho(x_i-t)}}{e^{\rho x_i}}  = g_{s,t}
	\]
