...
\begin{proof}
	...
	\begin{itemize}
		\item If both $x_k-s$ and $x_k-t$ are jump points, then we have $w(\eta_k +1) =\{T, T+1\}$, and there are ``constant differential" regions $n \in [S, S')$ and $n \in [T, T')$. We have shown that $w(n)$ will form a parallelogram that satisfy the formula.
...
\begin{proof}
	...
	&= \lim_{k\to\infty}\frac {x_{k-1}}{x_{k-1} + B +K_{s,t}\ln C }\\
	&= \lim_{k\to\infty}\frac {1}{1 + \frac{B +K_{s,t}\ln C}{x_{k-1}} } \\
	&=1
	\end{align*}
...
\end{lemma}
\begin{proof}
	Without loss of generality, we will only discuss for co-prime integer pair $(s, t)$. We will first prove a weaker limit about the joint sequence. Let's first look at the joint sequence $\{n_i\}_{i=\min\{s,t\}+1}^{\infty}$. 
	
	
	Recall the recurrence formula for the joint sequence 
	\[
	n_i = n_{i-s} + n_{i-t}
	\]
