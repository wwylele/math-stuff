...
\begin{proof}
	...
	Finally, let's show the constant differential between $n_{\triangleleft}$ and $n_{\triangleright}$. Considering three consecutive points $n, n+1, n+2$ ($n_{\triangleleft} \le n \le n_{\triangleright} - 2$), we can always find a $w^*$ such that
	\begin{align*}
	w^* &\in w(n)\\
...
There is more to say about the size of beads. In the previous property, we showed that two segments with constant slope, of length $p$ and $q$ respectively, will generate a bead also with constant slope of length $p + q$. This implies more beads with larger size will be generated, and the bead size $B(n)$ has the following relation
\[
	B(n) = B(w) + B(n-w)
\]
This hints the linear growth in bead size $B(n) \sim O(n)$.
...
\vspace{1cm}
\begin{lemma}[Rational]
	Given positive integer $s$ and $t$, define the non-decreasing sequence $\{n_i\}_{i=1}^\infty$ as
	\[
		n_1 = \dots = n_{s + t} = 1
	\]
	\[
	...
	Specifically
	\[
	w_{s,t}(n_i) = \{n_{i-t}\}\quad(n_i\geq 2)
	\]
	and $w_{s,t}(n)$ between $n_i$ and $n_{i-t}$ forms either a parallelogram or a straight thin line.
\end{lemma}
\begin{proof}
	Proof by induction
	\paragraph{Base case}
	The first no-empty $n$-interval is $n_{s+t} \le n <n_{s+t+1}$, which is exactly $n = 1$. We have shown that $\Delta E(1) = s + t$ so the base case satisfies.
	
	
	\paragraph{Inductive step} For all other non-empty $n$ interval $[n_i, n_{i+1})$ ($i > s + t$), both end satisfies the recurrence relation
	\begin{align*}
	n_{i} &= n_{i - s} + n_{i - t} \\
	n_{i + 1} &= n_{i- s + 1 } + n_{i - t + 1 } 
	\end{align*}
	Let's look at $E(n_{i})$ around $w = n_{i - t}$ (We will omit the special case for $n_{i-t} = 1$ or $n_{i-t} = n_{i} - 1$, as one side of the $w$-differential doesn't exist, but the argument for the minimal value is the same)
	\begin{align*}
	\Delta_w D^{n_{i - t} - 1}(n_{i}) &= \Delta E(n_{i - t} - 1) - \Delta E(n_{i} - n_{i - t}) + t - s \\ 
	&= \Delta E(n_{i - t} - 1) - \Delta E(n_{i - s}) + t - s \\ 
	&<  (i - t) - (i - s) + t - s  
	=0\\
	&\quad (\text{$[n_{i - s},n_{i - s} + 1)$ may be empty, but $\Delta E(n_{i - s})\geq i-s$ anyway})
	\end{align*}
	\begin{align*}
	\Delta_w D^{n_{i - t}}(n_{i}) &= \Delta E(n_{i - t}) - \Delta E(n_{i} - n_{i - t} - 1) + t - s \\ 
	&= \Delta E(n_{i - t}) - \Delta E(n_{i - s} - 1) + t - s \\ 
	&> (i - t) - (i - s) + t - s 
	=0
	\end{align*}
	So $w(n_{i}) = \{n_{i - t}\}$ (due to convexity), and 
	\begin{align*}
	E(n_i) &= E(n_{i - t}) + E(n_{i - s}) + n_{i - t} t + n_{i - s} s 
	\end{align*}
	With the same argument, at $E(n_{i + 1})$ we have  $w(n_{i + 1}) = \{n_{i - t + 1}\}$, and 
	\begin{align*}
	E(n_{i+1}) &= E(n_{i - t+1}) + E(n_{i - s+1}) + n_{i - t+1} t + n_{i - s+1} s 
	\end{align*}
	The total difference between the two is
	\begin{align*}
	E(n_{i+1}) - E(n_i) &= E(n_{i - t+1}) - E(n_{i - t}) + E(n_{i - s+1}) - E(n_{i - s}) + (n_{i - t+1} - n_{i-t}) t+ (n_{i - s+1} - n_{i-s}) s\\
	&= (n_{i - t+1} - n_{i-t})(i-t) + (n_{i - s+1} - n_{i-s})(i-s) + (n_{i - t+1} - n_{i-t}) t+ (n_{i - s+1} - n_{i-s}) s\\
	&=(n_{i - t+1} - n_{i-t} + n_{i - s+1} - n_{i-s})i\\
	&=(n_{i+1} - n_{i})i
	\end{align*}
	If $n_{i+1} - n_{i}$ = 1, then $E(n_{i+1}) - E(n_i) = \Delta E(n_i) = i$ and thus the formula is proved. Otherwise, let's look at $E(n_{i} + 1)$ around $w = n_{i - t}$ and $w = n_{i - t} + 1$ (again, ignore one side of the $w$-differential if it doesn't exist)
	\begin{align*}
	\Delta_w D^{n_{i - t} - 1}(n_{i} +1) &= \Delta E(n_{i - t} - 1) - \Delta E(n_{i} - n_{i - t} + 1) + t - s \\ 
	&= \Delta E(n_{i - t} - 1) - \Delta E(n_{i - s}  + 1) + t - s \\ 
	&< (i - t) - (i - s) + t - s 
	=0
	\end{align*}
	\begin{align*}
	...
	=0
	\end{align*}
	So the optimizer is among $w\in\{n_{i - t}, n_{i - t} + 1\}$ (or potentially both). Let's calculate their $D^w(n_{i}+1)$
	\begin{align*}
	D^{n_{i - t}}(n_{i}+1) &= E(n_{i - t}) + E(n_{i-s}+1) + n_{i - t} t + (n_{i-s}+1)s\\
	D^{n_{i - t} + 1}(n_{i}+1) &= E(n_{i - t}+1) + E(n_{i-s}) + (n_{i - t} + 1) t + n_{i-s}s\\
	\end{align*}
	So the differential at $E(n_{i})$ is
	\begin{align*}
	\Delta E(n_{i}) &= \min\{D^{n_{i - t}}(n_{i}+1) - E(n_i),D^{n_{i - t} + 1}(n_{i}+1)- E(n_i) \} \\
	&=\min\{\Delta E(n_{i-s}) + s, \Delta E(n_{i-t}) + t\}
	\end{align*}
	Remember $\Delta E(n_{i-s}) \ge i -s$, $ \Delta E(n_{i-t}) \ge i - t$, and at least one of them has equality, otherwise both $n_{i-s} = n_{i-s+1}$ and $n_{i-t} = n_{i-t+1}$, which means $[n_i, n_{i+1})$ is empty. So we have 
	\[
	\Delta E(n_{i}) = i
	\]
	Remember $E(n_{i+1}) - E(n_i) = (n_{i+1} - n_{i})i$, then we have
	\begin{align*}
	(n_{i+1} - n_{i})i &= E(n_{i+1}) - E(n_i) \\
		&= \Delta E(n_{i}) + \Delta E(n_{i} + 1) + \dots + \Delta E(n_{i+1} -1)\\
		&\le \Delta E(n_{i}) + \Delta E(n_{i}) +\dots +\Delta E(n_{i})\\
		&=(n_{i+1} - n_{i})i
	\end{align*}
	The equality must holds, so we have locked $\Delta E(n) = i$ for all $ n_i \le n < n_{i+1}$.
		
	Regarding the shape of $w(n)$ between $n_{i}$ and $n_{i+1}$, we have
	\begin{itemize}
		\item If $n_{i-s} \neq n_{i-s+1}$ and $n_{i-t} \neq n_{i-t+1}$, then $w(n_{i}+1) = \{n_{i - t}, n_{i - t} + 1\}$. Together with $w(n_{i}) =  \{n_{i - t}\}$, we know this forms a parallelogram as described before.
		\item If $n_{i-t} = n_{i-t+1}$, then the single value for $w$ at two ends $w(n_{i}) = w(n_{i+1})$. This restricts all $w$ to be on the same horizontal line
		\item If $n_{i-s} = n_{i-s+1}$, then the single value for $w$ at two ends $w(n_{i}) + n_{i+1} - n_i = w(n_{i+1})$. This restricts all $w$ to be on the same line with slope of 1.
	\end{itemize}
	
\end{proof}	

This property provides a way to recursively generate intervals of constant differential for $E(n)$ when $s$ and $t$ are integers (and by extension due to homogeneity, whenever $s/t$ is rational). For example, for $s = t = 1$ (balanced bisection), we have the following
\begin{align*}
\Delta E_{1,1}(1) &= 2 \\
...
\begin{proof}
	...
	We will use the $w$ formula from Property [Rational]. Remember we defined a helper sequence $\{n_i\}_{i=0}^{\infty}$. We first prove that, for co-prime $s$ and $t$ and large enough $i$, the interval has at least length of 2 (or any given number, but we don't need that)
	\[
	\exists I, \forall i>I, \Delta_i n_{i} \ge 2
	\]
	The differential of $\{n_i\}_{i=0}^{\infty}$ satisfies the recurrence relation
	\begin{align*}
	\Delta_i n_1 &= \Delta_i n_2 = \dots = \Delta_i n_{s+t-1} = 0\\
	\Delta_i n_{s+t} &= 1\\
	...
	\[
	\partial_{+t}E_{s,t}(n) = \partial_{-t}E_{s,t}(n) = \partial_{t}E_{s,t}(n) 
	\]
	This implies the derivative will have the unified formula
	\begin{align*}
	\partial_{t}E_{s,t}(1) &= 0\\
	\partial_{t}E_{s,t}(n) &= \partial_{t}E_{s,t}(w) + \partial_{t}E_{s,t}(n-w) + w\quad \forall w\in w_{s,t}(n) \quad(n\geq 2)
	\end{align*}
	We can also derive the secondary differential formula for $n\geq 2$
	\begin{align*}
	\Delta_n \partial_{t}E_{s,t}(n) =\begin{cases}
	\Delta_n  \partial_{t}E_{s,t}(n-w) & (w\in w_{s,t}(n), w\in w_{s,t}(n+1) )\\
	\Delta_n  \partial_{t}E_{s,t}(w) + 1 & (w\in w_{s,t}(n), w+1\in w_{s,t}(n+1) )
	\end{cases}
	\end{align*}
	...
	\begin{align*}
	\Delta_n \partial_{t}E_{s,t}(n_{i+t}) &= \Delta_n \partial_{t}E_{s,t}(n_{i + t-s}) \\
	\Delta_n \partial_{t}E_{s,t}(n_{i+t}) &= \Delta_n  \partial_{t}E_{s,t}(n_{i}) + 1
	\end{align*}
	Let's denote $\Delta_n \partial_{t}E_{s,t}(n_{i})$ as $d_i$. The equation above means, for all $i$ greater than a large enough $I$, we have
	\begin{align*}
	d_{i+s} &= d_{i}\\
	d_{i+t} &= d_{i} + 1
	\end{align*}
