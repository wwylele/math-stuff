...
&= \frac{1}{\sigma} + \frac{1}{\sigma} \sum_{j=0}^{\infty} (e^{-sr} + e^{-tr})^j\\
...
\begin{proof}
	...
	\[
	n_i = \sum_{j} c_j \xi^i_j
	\]
	where $\xi_j$ are the complex roots of the equation
	\[
	\xi^{-s} + \xi^{-t} = 1
	\]
	...
	\end{align*}
	This means the original equation for $\xi$ has a single positive real root, and it has the strictly largest magnitude among all complex roots. Notice that $\xi_* = e^{\rho_{s,t}}$ is a real root, so it is the root with the largest magnitude. 
	
	We have the following limit
	\begin{align*}
	\lim_{i\to\infty} \frac{n_{i-t}}{n_i} &= \lim_{i\to\infty} \frac{\sum_{j} c_j \xi^{i-t}_j}{\sum_{j} c_j \xi^i_j} \\
	&= \xi_*^{-t}\\
	&= e^{-t\rho_{s,t}}\\
	&= g_{s,t}
	\end{align*}
...
I haven't found a concrete proof yet, but here is some heuristic argument.

For $s/t\notin\mathbb{Q}$, we want to prove the limit
\[
\lim_{x\to\infty} \frac{\eta_{s,t}(x)}{e^{\rho_{s,t}x}} = \frac{1}{\rho_{s,t}(se^{t\rho_{s,t}}+te^{s\rho_{s,t}})}
\]
...
The left side function can be expanded using the series previously obtained via Laplace transform
\[
\frac{\eta_{s,t}(x)}{e^{\rho_{s,t}x}} =^{\star}  \sum_r \frac{e^{(r - \rho_{s,t})x}}{r(s e^{tr} + t e^{sr})}
\]
Remember $r= \rho_{s,t}$ is one of the root, so equivalently we want to prove the limit 
\[
\lim_{x\to\infty} \sum_{r \neq \rho_{s,t}} \frac{e^{(r - \rho_{s,t})x}}{r(s e^{tr} + t e^{sr})} = 0
\] 
Because all other solutions has $\mathfrak{R}(r) < \rho_{s,t}$, each individual term tends to zero. Unfortunately we can't simply swap the limit and summation because
\begin{itemize}
	\item The series doesn't converge absolutely.
	\item There are infinitely many $r$ whose real components are arbitrarily close to $\rho_{s,t}$, whose corresponding terms decay very slowly. This hints that summation of them at $x\to\infty$ might ``leak over" zero.
\end{itemize}
However, we also have heuristic data shows that the magnitude of each component is decreasingly smaller for closer $r - \rho_{s,t}$, limiting the contribution from slow-decaying terms to the series. Further more, the oscillating magnitude of the sum seems to be mainly determined by the term with the largest magnitude, which decreases and vanishes for $x\to\infty$, but not at an exponential rate overall when including all terms.
