...
\begin{lemma}[Fibonacci]
	...
	\[
		\Delta E_{s,t}(n) = i,\quad n_i \le n < n_{i+1}
	\]
\end{lemma}
\begin{proof}
	Proof by induction
	...
	\paragraph{Inductive step} For all other non-empty $n$ interval $[n_i, n_{i+1})$ ($i > s + t$), both end satisfies the iteration formula
	\begin{align*}
	n_{i} &= n_{i - s} + n_{i - t} \\
	...
	&<  (i - t) - (i - s) + t - s  
	=0\\
	&\quad (\text{$[n_{i - s},n_{i - s} + 1)$ may be empty, but $\Delta E(n_{i - s})\geq i-s$}) \\
	\end{align*}
	\begin{align*}
...
This property provides a way to iteratively generate intervals of constant differential for $E(n)$. For example, for $s = t = 1$ (balanced bisection), we have the following
\begin{align*}
\Delta E_{1,1}(1) &= 2 \\
...
\Delta E_{1,2}(\phi_{i -1}) =\dots = \Delta E_{1,1}(\phi_{i} - 1) &= i
\end{align*}
