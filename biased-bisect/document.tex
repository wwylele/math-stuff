...
\vspace{1cm}
\begin{lemma}[Pascal]
	$E(n)$ and $w(n)$ can be generated using binomial coefficients
\end{lemma}
\begin{proof}
	Consider the set of all binomial coefficients, tagged with a second component
\[
\Omega^E_{s,t} = \left\{\left(\binom{p+q-2}{p-1}, ps+qt \right) \ \middle|\ (p,q)\in(\mathbb{Z}^+)^2 \right\}
\]
Sort all elements by $ps+qt$ in ascending order. For equal elements, the order doesn't matter. Then starting from $n=1$, each element generates a segment in $E(n)$ with the length of $\binom{p+q-2}{p-1}$, and slope ($\Delta n$) of $(ps+qt)$.

Similarly, consider the set of all binomial coefficient pairs, again tagged. Here we use the extension that $\binom{n}{-1} = \binom{n}{n+1} = 0$
\[
\Omega^w_{s,t} = \left\{\left(\binom{p+q-3}{q-2}, \binom{p+q-3}{p-2}, ps+qt \right) \ \middle|\ (p,q)\in(\mathbb{Z}^+)^2 \backslash (1,1) \right\}
\]
Sort all elements by $ps+qt$ in ascending order. This time for equal elements, we combine them into one element by calculating the sum of $\binom{p+q-3}{q-2}$ and $\binom{p+q-3}{p-2}$. We then connect all vectors $\left(\binom{p+q-3}{q-2}, \binom{p+q-3}{p-2}\right)$ head to tail, starting from $(1,1)$. Each vector becomes a segment $(w_{\triangleleft}, w'_{\triangleleft}) - (w_{\triangleright}, w'_{\triangleright})$, each corresponds to a bead where
\begin{align*}
...
Each bead in the $w(n)$ graph is like expanding a segment in the $w$-$w'$ graph into a rectangle, treating the segment as a diagonal. We can also choose to not to expand into rectangles but to keep the segments. These segments defines a function $w^{\ominus}(n)$. It can be seen that we always have
\[
w^{\min}(n)\le w^{\ominus}(n)\le w^{\max}(n)
\]

The following is the $w$-$w'$ graph of $w^{\ominus}$ for $s=4$, $t=9$
...
\end{lemma}
\begin{proof}
	We will show that, fixing $n$ and $s$, $w^{\ominus}_{s,t}(n)$ is a non-increasing function of $t$, and then we can say $w^{\star}_{s,t}(n) = \lfloor w^{\ominus}_{s,t}(n)\rfloor$ satisfies our proposal.
