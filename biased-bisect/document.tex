...
\]
$w_{s,t}(n)$ being a simple interval trivially follows the convexity proposition, so we will focus on proving convexity.
...
Using the notation $D^w_{s,t}(n) = E_{s,t}(w) + E_{s,t}(n-w) +wt+(n-w)s$ we have
\[
...
\Delta E(n-1)&= \begin{cases}
\Delta E(w(n-1)) + t \quad(w(n) = w(n-1) + 1)\\
\Delta E(n-1-w(n-1)) + s\quad(w(n) = w(n-1))
\end{cases} \quad (\text{Proposition $\mathbf{W}_{n-1}$}) 	
\end{align*}
There exists $w(n)\geq w(n-1)$ (Proposition $\mathbf{W}_{n-1}$), so $\Delta E(w(n)) + t \geq  \Delta E(w(n-1)) + t$ (Proposition $\mathbf{C}_n$). Similarly, there exists $n-w(n)\geq n-1-w(n-1)$ (Proposition $\mathbf{W}_{n-1}$), so $\Delta E(n-w(n)) + s \geq \Delta E(n-1-w(n-1)) + s$ (Proposition $\mathbf{C}_n$). Combining these, we have $\Delta E(n-1) < \Delta E(n)$, proving proposition $\mathbf{C}_{n+1}$
...
\end{lemma}
\begin{proof}
	First of all, the two statements are equivalent under the symmetry property:
	\[
	...
	From previous properties, we know that $w^{min}(n+1) \leq w^{min}(n) + 1$, because otherwise there would be no value $w(n+1)$ that can be either $w^{min}(n)$ or $w^{min}(n) + 1$. Now we only need to prove $w^{min}(n+1)\geq w^{min}(n)$, or in other words, $w^{min}(n) - 1 \notin w(n+1)$. This is obviously true for $w^{min}(n) = 1$, so let's discuss when $w \geq 2$. We have the following differential
	\begin{align*}
	\Delta^w D^{w^{min}(n)-1}(n) &= \Delta E(w^{min}(n)-1) - \Delta E(n-w^{min}(n)) + t - s\\
	\Delta^w D^{w^{min}(n)-1}(n+1) &= \Delta E(w^{min}(n)-1) - \Delta E(n+1-w^{min}(n)) + t - s
	\end{align*}
	Due to convexity of $E(n)$, we have $\Delta^w D^{w^{min}(n)-1}(n) \geq 	\Delta^w D^{w^{min}(n)-1}(n+1)$. At the same time, we also have $\Delta^w D^{w^{min}(n)-1}(n) < 0$ (otherwise $w^{min}(n)$ should be smaller due to convexity of $D^w$), so we have $\Delta^w D^{w^{min}(n)-1}(n+1) < 0$, meaning $w^{min}(n) -1$ can't be a minimal point of $D^{w}(n+1)$. Therefore we finished the prove.
\end{proof}
